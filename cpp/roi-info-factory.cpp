...
bool RoiInfoFactory::sanityChecks(const unsigned long end_row, const unsigned long end_col){
    ...
    if(result)
    {
        std::cout << "rows" << std::to_string(this->rows) << "cols" << std::to_string(this->cols) << std::endl;
        std::cout << "end_row" << std::to_string(end_row) << "end_cols" << std::to_string(end_col) << std::endl;
    }
    return result;
}